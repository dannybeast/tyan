  ...
  $.fn.betterDropdown = function(options) {
    ...
    function buildDropdown(originalDropdown) {
      ...
      var $dropdownBox = $(
        '<div class="' +
          className +
          "-dropdown-box-wrapper dropdown-box-wrapper " +
          id +
          '">\
                        <span class="' +
          className +
          '-dropdown-box-text dropdown-box-text">Select ' +
          itemName +
          "</span>\
                    </div>"
      );
      ...
      var $downArrow = $(
        '<div class="down-arrow ' +
          className +
          '-dropdown-icon dropdown-icon"></div>'
      );

      $downArrow.appendTo($dropdownBox);

      $dropdownBox.appendTo(parentContainer).click(function() {
        toggleDropdown();
      });

      var $filterWrapper = $(
        '<div class="' + className + '-filter-wrapper filter-wrapper"></div>'
      );
      ...
      var $filterResultsWrapper = $(
        '<div class="' +
          className +
          '-filter-results-wrapper filter-results-wrapper"></div>'
      );
      ...
      $.each($("#" + id).children(), function(i, option) {
        ...
        if (settings.displayTextBelow) {
          ...
          $resultTextContainer.append($lineBreak).append($resultLastText);
        }
        ...
        if (option.value !== "") {
          var $filterResult = $(
            '<div class="' + className + '-result result"></div>'
          );
          $filterResult
      ...
      if ($("#" + id).val() && $("#" + id).val() !== "") {
        selectItem(
          $("." + className + "-result").filter(function() {
      ...
      function selectItem($item) {
        ...
        $("." + className + "-dropdown-box-text").text(
          $item
            .children()
            .first()
            .children()
        ...
        $("#" + id + ' option[value="' + $item.data().value + '"]').prop(
          "selected",
          true
        );
        $("#" + id).trigger("change");
      ...
      function filterItems(filterText) {
        ...
        $.each(
          $("." + className + "-filter-results-wrapper").children(),
          function(i, option) {
            var $option = $(option);
            ...
            var inDisplayName =
              $option.data("filter-text").indexOf(filterText) !== -1;
            var inValue =
              $option.data("filter-value").indexOf(filterText) !== 1;

            var inBelowText = false;

            if (
              $(option).data("displayBelowText") &&
              $(option)
                .data("displayBelowText")
        ...
        if (
          $("." + className + "-filter-results-wrapper")
            .children()
            .filter(":visible").length === 0
        ) {
          var $itemNotFoundMessageContainer = $(
            '<div class="' + className + '-no-result-message"></div>'
          );

          $("<i></i>")
            .text("No " + itemName.toLowerCase() + "s found.")
            .appendTo(
              $itemNotFoundMessageContainer.appendTo($filterResultsWrapper)
            );
        }
      }
      ...
      function toggleDropdown() {
        ...
        if ($filterWrapper.css("display") === "none") {
          ...
          $filterWrapper.show().css("z-index", 1);
      ...
      function closeDropdown() {
        $filterWrapper.hide().css("z-index", "auto");
      ...
      function flipDropdownArrow() {
        ...
        if ($dropdownIcon.hasClass("down-arrow")) {
          $dropdownIcon.addClass("up-arrow").removeClass("down-arrow");
        } else {
          $dropdownIcon.addClass("down-arrow").removeClass("up-arrow");
        }
      }
      ...
      function arrowKeyNavigate(e) {
        ...
        var $nextItem = $hoveredItem
          ...
          $prevItem = $hoveredItem
            ...
            .filter(":visible")
            .first(),
          $firstItem = $filterResultsWrapper
            .children()
            .filter(":visible")
            .first(),
          $lastItem = $filterResultsWrapper
            .children()
            .filter(":visible")
            .last();
      ...
      function scrollFilterResultsWrapper($hoveredItem) {
        ...
        var resultsWrapperTop = $(
          "." + className + "-filter-results-wrapper"
        ).offset().top;
        var resultsWrapperBottom =
          resultsWrapperTop +
          $("." + className + "-filter-results-wrapper").height();
        ...
        if (itemOffscreenBottomValue > 0) {
          $filterResultsWrapper.scrollTop(
            $filterResultsWrapper.scrollTop() + itemOffscreenBottomValue
          );
        }
        ...
        if (itemOffscreenTopValue > 0) {
          $filterResultsWrapper.scrollTop(
            $filterResultsWrapper.scrollTop() - itemOffscreenTopValue
          );
        }
      }
